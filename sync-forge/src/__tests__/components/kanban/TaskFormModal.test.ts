...
describe(`TaskFormModal.vue`, () => {
  describe(`Create Mode`, () => {
    it(`renders with "New Task" title and empty inputs`, () => {
  ...
  describe(`Edit Mode`, () => {
    it(`renders with "Edit Task" title and pre-filled inputs`, () => {
      ...
      expect((wrapper.find(`textarea`).element as HTMLTextAreaElement).value).toBe(MOCK_TASK.description)
    })
    ...
    it(`emits save with updated data`, async () => {
      ...
      expect(saveEvent).toHaveLength(1)
      expect(saveEvent?.[0]).toStrictEqual([{ title: `Updated Title`, description: MOCK_TASK.description }])
    })
  })
})
